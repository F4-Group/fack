...
////////////////////////////////////////////////////////////////////////////////
    function registerBrowserifyBundles(cb) {
        ...
        _.each(app.get("jsEntries"), function (jsEntryDir) {
            var bundleConfiguration = {jsEntryDir};
            if (!_.isString(jsEntryDir)) {
                bundleConfiguration = _.clone(jsEntryDir);
            }

            var jsEntriesPattern = path.join(bundleConfiguration.jsEntryDir, '**', '*.js');
    ...
    function createBrowserifyBundlesMiddleware() {
        ...
        function middleware(req, res, next) {
            if (buildBundleOnFirstRequest) {
                ...
                var path = pathUtil.getPathWithoutPrefix(req, "");
                var bundleHandler = bundleHandlers[path];
                if (bundleHandler)
...
////////////////////////////////////////////////////////////////////////////////
    function createBundle(bundleConfiguration, cb) {
        ...
        if ('jadeify' in bundleConfiguration) { // backward compatibility
            bundleConfiguration.pugify = bundleConfiguration.jadeify
            logger.warn('jadeify option is deprecated, use pugify instead');
            delete bundleConfiguration.jadeify;
        }
        var transforms = _.clone(bundleConfiguration.transforms);
        ...
        function getBundle(bundleConfiguration) {
            ...
            var babelifyTransformOptions = _.extend({presets: ["es2015"]}, options.babelify, bundleConfiguration.babelify);
            if ('babelifyIgnore' in bundleConfiguration) {// for backward compatiblity
                logger.warn('babelifyIgnore bundle option is deprecated, use babelify:{ignore: ...} option instead');
                babelifyTransformOptions.ignore = bundleConfiguration.babelifyIgnore;
            }
            bundle.transform("babelify", babelifyTransformOptions);
            bundle.transform(aliasify, {global: true, aliases: requireAliases});
        ...
        function factoredStream(fileName, cb) {
            ...
            writableStream.pipe(concat(function (src) {
                ...
                registerJs(fileName, src, sourceMap);
                if (cb)
                    cb(null);
            }));
