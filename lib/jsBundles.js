...
const bundleFile = require('./bundleFile');
...
function jsBundles(app, options, cacheforeverMiddleware, i18nOptions) {
    ...
    function createBundle(bundleConfiguration, cb) {
        ...
        const mostSpecificPugifyOption = _.has(bundleConfiguration, 'pugify') ? bundleConfiguration.pugify : options.pugify;
        if (mostSpecificPugifyOption !== false) {
            transforms.push(pugify.pug(_.extend({
                pretty: false,
                compileDebug: pugDebug,
            }, options.pugify, bundleConfiguration.pugify)));
        }
        ...
        if (bundleFile.canLoad(name, options)) {
            loadBundle(name, bundleConfiguration);
            setImmediate(cb);
        } else {
        ...
        function buildUsingFactorBundle(bundle, cb) {
            ...
            logger.info('Building factored bundle %s (with %s)...',
                bundleConfiguration.name, bundleConfiguration.factorBundleNames);
            const outputs = [];
        ...
        function factoredStream(bundleName, cb) {
            ...
            writableStream.pipe(concat(function (src) {
                ...
                if (bundleConfiguration.minify && bundleConfiguration.enableMinifyGlobal) {
                    compressJs(src, sourceMapJson, bundleName, function (result) {
                        src = result.code;
        ...
        function build(bundle, cb) {
            ...
            stream.pipe(concat(function (src) {
                src = src.toString();
                if (bundleConfiguration.minify && bundleConfiguration.enableMinifyGlobal) {
                    compressJs(src, sourceMapJson, name, function (result) {
                        src = result.code;
                        sourceMapJson = result.map;
                        processSrc();
                    });
        ...
        function registerJs(name, src, sourceMap) {
            const {saveBundles, bundlesCache} = options;
            if (saveBundles) {
                bundleFile.save(bundlesCache, name, src);
                if (sourceMap) {
                    bundleFile.save(bundlesCache, name + '.map', sourceMap);
                }
            }
        ...
        function loadBundle(name) {
            if (bundleConfiguration.factorBundles) {
                _.each(bundleConfiguration.factorBundles, ({name: factorBundleName}) => {
                    const src = bundleFile.load(factorBundleName, options);
                    const sourceMap = bundleFile.load(factorBundleName + '.map', options);
                    registerJs(factorBundleName, src, sourceMap);
                });
            }
            const src = bundleFile.load(name, options);
            const sourceMap = bundleFile.load(name + '.map', options);
            registerJs(name, src, sourceMap);
        }
    }

    function compressJs(src, sourceMapJson, name, cb) {
        minify(src, {
            sourceMap: {
