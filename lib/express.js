...
const appNameHeaderMiddleware = require('./middlewares/appNameHeader');
...
function expressWrapper(options) {
    ...
    options = _.defaults(options || {}, {
        ...
        bundlesCache: path.join(rootDir, 'bundles'),
        preferCachedBundles: isProd,
        preconfigure: null,
        bundles: [],
        minify: minify,
    ...
    app.use(appNameHeaderMiddleware);
    ...
    app._buildBundles = _buildBundles;
    return app;
        ...
        getStylusFiles(function (err, files) {
            if (err) {
                ...
                async.eachSeries(files, function (file, cb) {
                    getCssContent(file, (err, css) => {
                        if (err) {
                            logger.error(err, 'compileStylusFiles failed for', file);
                        } else {
                            registerCss(getCssPathFromStylus(file), css);
                        }
                        cb(err);
                    });
                }, callback);
    ...
    function getCssPathFromStylus(stylusFileName) {
        return filePathToStaticUriPath(viewsDir, stylusFileName).replace(/\.styl$/, '.css');
    }

    function getCssContent(file, cb) {
        const cssFileName = getCssPathFromStylus(file);
        if (bundleFile.canLoad(cssFileName, options)) {
            const cssContent = bundleFile.load(cssFileName, options);
            cb(null, cssContent);
        } else {
            const startDate = Date.now();
            logger.info('Compiling %s', file);
            const stylusContent = fs.readFileSync(file, {encoding: 'utf8'});
            compileStylus(stylusContent, file).render(function (err, css) {
                if (err) {
                    cb(err);
                } else {
                    ...
                    logger.info({timeInMs: durationInMs, size},
                        'Compiled %s in %ds (%s)', file, durationInMs / 1000, humanize.filesize(size));
                    cb(null, css);
                }
            });
        }
    ...
    function registerCss(cssFileName, css) {
        const {saveBundles, bundlesCache} = options;
        if (saveBundles) {
            bundleFile.save(bundlesCache, cssFileName, css);
        }
        cacheforeverMiddleware.register({
            path: cssFileName,
            content: css,
    ...
    function registerErrorHandler(cb) {
        app.use(function (err, req, res, next) {
            if (err) {
                req.logger.error(err, 'Unhandled error on %s at %s', req.method, req.url);
            }
