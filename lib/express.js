...
function expressWrapper(options) {
    ...
    options = _.defaults(options || {}, {
        ...
        globalpasswords: [],
        globalpasswordMiddleware: null,
        //null middle ware means use default, false middleware means disable, can also set a custom middleware
        jsonMiddleware: null,
        urlencodedMiddleware: null,
        multipartMiddleware: null,
    });
    ...
    var globalpasswordMiddleware = options.globalpasswordMiddleware;
    if(!globalpasswordMiddleware && options.globalpasswords && options.globalpasswords.length) {
        globalpasswordMiddleware = globalpassword({
            passwords: options.globalpasswords,
        })
    }
    if(globalpasswordMiddleware)
        options.session = true;
    if(options.session)
        options.cookie = true;

    if(!options.jsonMiddleware && options.jsonMiddleware !== false)
        options.jsonMiddleware = express.json();
    if(!options.urlencodedMiddleware && options.urlencodedMiddleware !== false)
        options.urlencodedMiddleware = express.urlencoded();
    if(!options.multipartMiddleware && options.multipartMiddleware !== false)
        options.multipartMiddleware = express.multipart();
    ...
    app.configure(function () {
        ...
        if(options.cookie)
            app.use(express.cookieParser(options.cookieSecret || null));
        if(options.session) {
    ...
    app.start = start;
    app.packageJavascript = function (path, cb) {
        createBundle({
            serve: false,
            path: path,
            minify: true,
