    ...
    function start(cb) {
        ...
        function listen(err) {
            if (err) {
                ...
                if (watch) {
                    debugIo = socketio(server, {
                        path: constants.debugSocketIoPath,
                    });
                }
                if (cb)
                    cb(server);
...
////////////////////////////////////////////////////////////////////////////////
    function jsUrl(path) {
        return cacheforeverMiddleware.resourceUrl(path);
    }

////////////////////////////////////////////////////////////////////////////////
    function cssUrl(path) {
        return cacheforeverMiddleware.resourceUrl(stylusSubDir + '/' + path);
    }

////////////////////////////////////////////////////////////////////////////////
    function watchStylusFolder(dir) {
        var watcher = chokidar.watch(path.join(dir, '/**/*.{styl,css}'), {
            ignoreInitial: true,
        });
        watcher
            .on('add', reloadCss)
            ...
            .on('change', reloadCss)
            .on('unlink', reloadCss)
            .on('unlinkDir', reloadCss)
            .on('error', function (error) {
                logger.warn('Watching stylus files: %s', error);
            });

        function reloadCss() {
            debugIo.emit('reloadCss');
        }
    }
...
function getRequireAliases() {
    ...
    _.each(jsFiles, function (jsFile) {
        if (/\.js$/.test(jsFiles)) {
            aliases['bootstrap-' + jsFile.replace(/\.js$/, '')] = path.join(bootstrapJsDir, jsFile);
        }
    });
    return aliases;
}
