...
var requireAliases = (function() {
    var aliases = {
        ...
        'fackResourceUrl': path.join(__dirname, 'require/fackResourceUrl.js')
    };
...
function expressWrapper(options) {
    ...
    if (options.errormethodMiddleware === true)
        options.errormethodMiddleware = f4ExpressMiddlewares.errorMethod(logger);
    ...
    app.configure(function() {
        ...
        app.use(function(req, res, next) {
            ...
            // todo: check all accepted languages to find the first supported one
            language = language.split(',')[0];
            language = language.split(';')[0];
            language = language.split('-')[0];
            language = language.split('_')[0];
            language = i18n.functions.toLanguages(language)[0];
            var translateOptions = {
                lng: language,
            };
            app.locals.language = language;
            app.locals.t = function(key, opts) {
                if (opts)
                    opts.lng = language;
                else
    ...
    app.start = start;
    app.packageJavascript = function(bundleConfiguration, cb) {
...
////////////////////////////////////////////////////////////////////////////////
    function getJsEntries() {
        ...
        try {
            ...
            logger.debug('Could not read JS entries from %s', jsEntryDir, e);
        }
...
////////////////////////////////////////////////////////////////////////////////
    function createBundle(bundleConfiguration, cb) {
        ...
        var name = path.basename(bundleConfiguration.path);
        ...
        function getBundle(getBundleOptions) {
            var bundle = browserify({
                ...
                resolve: function(id, opts, callback) {
                    id = options.requireAliases[id] || requireAliases[id] || id;
            ...
            bundle.bundle({
                ...
                insertGlobalVars: _.extend({
                    ...
                    resourceUrl: function() {
                        return "require('fackResourceUrl')";
                    },
                }, insertGlobalVars),
            }, function(err, src) {
                if (err) {
