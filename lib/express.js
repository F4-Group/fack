 ...
 * @property {string[]} factorBundles use factor-bundle plugin. this will generate a package with everything that is common to the given packages, removing it from those packages
 */
/**
 *
 * @param {object} options
 * @param {boolean} options.crossdomainMiddleware add crossdomain meta
...
function expressWrapper(options) { // jshint ignore:line
    ...
    addOptionalMiddleware('crossdomainMiddleware', function () {
...
////////////////////////////////////////////////////////////////////////////////
    function registerBrowserifyBundles(cb) {
        ...
        var bundles = _.map(options.bundles,
            function (bundlePath) {
                if (_.isString(bundlePath)) {
                    manuallyConfiguredBundles.push(bundlePath);
                    return {
                        name: useNativePathSep(path.relative(defaultEntryDir, bundlePath)),
                        path: bundlePath,
                    };
                } else {
                    if (!bundlePath.path && !bundlePath.requires && !bundlePath.factorBundles) {
                        if (!bundlePath.entryPoint)
                            throw new Error("bundleConf without entryPoint and path: " + JSON.stringify(bundlePath));
                        else
                            bundlePath.path = useNativePathSep(path.resolve(defaultEntryDir, bundlePath.entryPoint + ".js"));
                    }
                    if (!bundlePath.name) {
                        if (!bundlePath.path)
                            throw new Error("bundleConf without path and name: " + JSON.stringify(bundlePath));
                        else
                            bundlePath.name = useNativePathSep(path.relative(defaultEntryDir, bundlePath.path));
                    }
                    manuallyConfiguredBundles.push(bundlePath.path);
                    return bundlePath;
                }
            });
        ...
        _.each(app.get("jsEntries"), function (jsEntryDir) {
            ...
            bundles = bundles.concat(_.map(
                _.filter(_.map(entries, useNativePathSep), function (entry) {
...
////////////////////////////////////////////////////////////////////////////////
    function createBundle(bundleConfiguration, cb) {
        ...
        if (!name) {
            name = useNativePathSep(path.relative(app.get("jsEntries")[0], bundleConfiguration.path));
        }
        ...
        function getBundle(bundleConfiguration) {
            ...
            addGeneratedRequire(bundle, 'fack/cacheforever', cacheforeverMiddleware.getBrowserifyModule());
            var fackOptions = {
                i18next: i18nOptions || {},
            };
            addGeneratedRequire(bundle, 'fack/options', 'module.exports=' + JSON.stringify(fackOptions) + ';');
            if (!watch && options.moduleDepsCache) {
                options.moduleDepsCache.configure(bundle);
            }
...
function addGeneratedRequire(bundle, name, content) {
    // see https://gist.github.com/terinjokes/9e95ea98d41919266fbb
    bundle.exclude(name);
    bundle.require(stringStream(content), {
        expose: name,
        file: name + '.js',
    });
}
