 ...
 * @property {string} path optional only if require or factorBundles is defined
...
////////////////////////////////////////////////////////////////////////////////
    function registerBrowserifyBundles(cb) {
        ...
        var bundles = _.map(options.bundles,
            function (bundlePath) {
                if (_.isString(bundlePath)) {
                    ...
                    if (!bundlePath.path)
                        throw new Error("bundleConf without path : " + JSON.stringify(bundlePath));
                    if (!bundlePath.name)
        ...
        _.each(app.get("jsEntries"), function (jsEntryDir) {
            ...
            //Todo unique following paths
            bundles = bundles.concat(_.map(
                _.map(entries, useNativePathSep),
                function (bundlePath) {
                    return {
