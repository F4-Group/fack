...
proto.middleware = function() {
    ...
    return function(req, res, next) {
        ...
        if (req.url.indexOf(staticPath) == 0) {
            var path = getPathWithoutPrefix(req, staticPath);
            asset = assets[path];
            if (asset)
                maxAge = staticMaxAge;
            ...
            var path = getPathWithoutPrefix(req, prefix);
            asset = assets[path];
            if (asset)
                maxAge = foreverMaxAge;
        }
        if (asset)
            sendAsset(res, asset, staticMaxAge);
        else
            next();
    };
};
...
proto.getBrowserifyModule = function() {
    ...
    return JSON.stringify(exportedCacheforever) + ";";
};
